...
The Context API provides read access to the current state of the Contena Administration. Extensions can use these methods to retrieve information about the active language, locale, currency, environment, Contena version, and more.

This is useful for adapting extension behavior based on the current Administration context — for example, loading translations for the active language, matching the active color theme, or checking the Contena version before using a newer API.

```ts
import { context } from "@contena/meteor-admin-sdk";
```
...
Returns the system currency configured for the Contena instance. Use this when displaying prices or working with monetary values.
...
Returns the Contena version as a string. Use this to conditionally enable features or check compatibility before using newer APIs.

#### Usage

```ts
const contenaVersion = await context.getContenaVersion();
```
...
## compareIsContenaVersion()

Compares the current Contena version against a target version. The current Contena version is always the left-hand side of the comparison — so `context.compareIsContenaVersion('>=', '7.0.0')` reads as "is the current Contena version equal to or greater than 7.0.0?"

#### Usage

```ts
const isRightVersion = await context.compareIsContenaVersion(">=", "7.0.0");
```
...
The function supports both Contena's four-digit version number and semver versions. The following calls are equivalent:

```ts
await context.compareIsContenaVersion(">=", "6.6.4.0");

await context.compareIsContenaVersion(">=", "6.4.0");
```
...
> The `privileges` property is available since Contena v6.7.1.0.
...
> Available since Contena v6.4.9.0
...
> Available since Contena v6.6.2.0
...
```ts
import { window as swWindow } from "@contena/meteor-admin-sdk";

const { modules } = await context.getModuleInformation();

swWindow.routerPush({
  name: "ct.extension.sdk.index",
  params: {
    id: modules[0].id,
  },
});
...
Returns the unique shop ID used by Contena's app system. Use this to identify the shop instance when communicating with external services.

> Available since Contena v6.7.1.0
