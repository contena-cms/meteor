    ...
    <div class="IconSelection_sidebar-bg">
      ...
      <div class="flex flex-col gap-4">
        <div class="flex gap-4 items-center">
          <CtIcon class="--medium" :icon="icon.name" :type="icon.mode" />
          <CtIcon class="--small" :icon="icon.name" :type="icon.mode" />
        </div>

        <div class="flex gap-4 justify-center">
          <CtIcon class="--large" :icon="icon.name" :type="icon.mode" />
        </div>
      </div>
    </div>
...
.IconSelection {
  ...
  &_tag {
    @apply bg-[var(--ct-c-gray-100)] text-[var(--ct-c-gray-dark-500)] text-sm px-2 py-1;
  }
  ...
  &_sidebar-bg {
    @apply p-6 bg-[var(--ct-c-gray-50)];
    .dark & {
      @apply bg-[var(--ct-c-gray-dark-700)];
    }
  }
...
const exampleHTML = computed(() =>
  props.icon ? `<ct-icon name="${props.icon.mode}-${props.icon.name}" />` : null
);
