...
describe("mt-select", () => {
  ...
  it("should select an item with a pointer", async () => {
    const wrapper = mount(MtSelect, {
      props: {
        modelValue: null,
        options: [
    ...
    await wrapper.find(".mt-select__selection").trigger("click");
    await flushPromises();
    const results = document.querySelectorAll(".mt-select-result");
    results[1].dispatchEvent(new MouseEvent("pointerdown", { bubbles: true }));
    await flushPromises();
