...
A new token can be initiated by anyone. A designer might identify a gap while building a new component; an engineer might notice a hardcoded value that should be abstracted. Either way, the proposal should be shared with the other discipline before work begins. Use a [GitHub issue](https://github.com/contena/meteor/issues) or the Meteor Slack channel to open the discussion.
...
Once approved, update the token dictionaries and generated CSS deliverables together in the token package. The Meteor team reviews the pull request and publishes the token in the next release.
