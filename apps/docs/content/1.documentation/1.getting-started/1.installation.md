...
```text
Install @contena/meteor-component-library in this project. Connect the Meteor MCP server at https://meteor.contena.cn/mcp (or read https://meteor.contena.cn/llms.txt) to learn the components and conventions.
```
...
```sh
npm install @contena/meteor-component-library
```
...
Chinese and English translations are bundled. Register `vue-i18n` before mounting your app.
...
```vue
<script setup>
import { MtButton, MtBanner } from "@contena/meteor-component-library";
</script>
...
```ts
import PlusIcon from "@contena/meteor-icon-kit/icons/regular/plus.svg";
```
