...
const servicePaymentModalLocationId = 'ct-service-payment-modal';
...
export const addPaymentIframe = async (
  ...
  const link = `${baseUrl}/payment?service-name=${name}&service-version=${version}&shop-url=${shopUrl}&ct-version=${options.swVersion}&ct-user-language=${options.swUserLanguage}&shop-plan=${shopPlan}&shop-id=${shopId ?? ''}`;
