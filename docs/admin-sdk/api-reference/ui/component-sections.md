...
| `priority`   | false    |         | Controls the render order when multiple extensions register a component at the same `positionId`. Available since Contena v6.7.14.0. Lower values render first (`1` = topmost). Must be a positive number (`1` or higher); omitted or invalid values render after entries that set a `priority`. Sections registered by Contena services always render above app sections. |
