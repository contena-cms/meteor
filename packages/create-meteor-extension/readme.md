# @contena/create-meteor-extension

A CLI tool for scaffolding new Meteor Admin SDK extensions for Contena 6. This tool generates a complete project setup with Vue 3, TypeScript, and Vite, allowing you to quickly start building custom extensions for the Contena Administration.

## What is Meteor?

Meteor is Contena's Admin SDK that enables developers to extend the Contena Administration with custom UI components and functionality without modifying the core. Extensions built with Meteor run in isolated iframes and communicate with the Contena Administration through a secure messaging system.
...
- **Internationalization**: Built-in i18n support with example translations (Chinese and English)
- **Component Library**: Integrates `@contena/meteor-component-library` for consistent UI
...
npx @contena/create-meteor-extension

# Or install globally
npm install -g @contena/create-meteor-extension
...
**Note**: The CLI will always create a folder named `meteor-app` (as required by Contena 6.7+ plugin structure), but the extension name you provide will be used in `package.json`, README, and other configuration files.
...
```bash
npx @contena/create-meteor-extension --name my-extension --output-dir meteor-app
```
...
npx @contena/create-meteor-extension --name my-awesome-extension

# Create extension with custom output directory
npx @contena/create-meteor-extension --name my-extension --output-dir custom-folder
```
...
For Contena 6.7+ plugins, this should be placed in `custom/plugins/yourPlugin/src/Resources/app/meteor-app`.

### Key Dependencies

- **@contena/meteor-admin-sdk**: Core SDK for Contena Admin extensions
- **@contena/meteor-component-library**: UI component library matching Contena's design system
...
3. Install and activate the Extension in Contena.
...
1. **Dashboard Card** (`example-dashboard-before-content`): Appears on the Contena dashboard
...
- Node.js 18+
- npm or pnpm
- Contena 6 instance for testing your extension
