import getCompareIsContenaVersion from './compare-version';

type Comparator = '=' | '!=' | '<' | '>' | '<=' | '>=';

describe('getCompareContenaVersion', () => {
  const mock = (version: string) => () => Promise.resolve(version);

  describe('It works with contena versions and semver versions', () => {
    it.each<[boolean, string, Comparator, string]>([
      ...
      [false, '6.8.2', '=', '6.6.9.2'],
      [false, '6.8.2', '=', '6.9.2'],
    ])("returns %s for '%s %s %s'", async (exptectedResult, contenaVersion, comparator, comparedVersion) => {
      const compareIsContenaVersion = getCompareIsContenaVersion(mock(contenaVersion));

      expect(await compareIsContenaVersion(comparator, comparedVersion)).toBe(exptectedResult)
    });
  });
