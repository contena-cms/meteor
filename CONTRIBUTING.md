...
Contena is available under [MIT license](https://github.com/contena/meteor/blob/main/LICENSE.md).
...
our [code of conduct](https://github.com/contena/meteor/blob/main/CODE_OF_CONDUCT.md).

## Bugs

Please take a look at the [existing issues](https://github.com/contena/meteor/issues)
...
Search through the [list of existing feature requests](https://github.com/contena/meteor/issues),
...
you can [request a feature](https://github.com/contena/meteor/issues/new?template=03-feature.yaml).
...
To start just one of them, run `pnpm --filter @contena/meteor-component-library dev` for Storybook or `pnpm --filter meteor-docs dev` for the documentation.
...
To use your changes in Contena and your extensions, you first build and publish the changed package, then add it to your project in a second step.
...
2. Build from the meteor root so the published files include your changes:
   ...
   To rebuild only one package, scope the task: `npx turbo run build --filter=@contena/meteor-component-library`.
...
### Add your updated package to Contena and your extensions

Switch to the project that should consume your changes. For the Contena administration this is `<contena-root-folder>/src/Administration/Resources/app/administration`:

```sh
cd <contena-root-folder>/src/Administration/Resources/app/administration
yalc add @contena/meteor-component-library
npm install
```
...
Alternatively, run `yalc link @contena/meteor-component-library` to symlink the files into `node_modules` without touching `package.json`.
...
2. Go to the [actions tab](https://github.com/contena/meteor/actions)
3. Click on [Visual tests](https://github.com/contena/meteor/actions/workflows/visual-tests.yml) in the left sidebar
