...
export const VisualTestRenderLinkWithoutNewTab: MtTextEditorStory = defineStory({
  ...
  args: {
    modelValue:
      '<p>Visit our <a href="https://www.contena.cn">shop page</a> for more information.</p>',
  },
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
...
export const SetLink: MtTextEditorStory = defineStory({
  ...
  play: async ({ canvasElement, args }) => {
    ...
    // Check if args was triggered with new content
    expect(args.updateModelValue).toHaveBeenCalledWith(
      '<h1><a target="_blank" rel="noopener noreferrer nofollow" href="https://www.contena.cn">Hello World</a></h1><p>Some text</p>',
    );
  },
});
...
export const SetLinkWithoutNewTab: MtTextEditorStory = defineStory({
  ...
  play: async ({ canvasElement, args }) => {
    ...
    expect(args.updateModelValue).toHaveBeenCalledWith(
      '<h1><a href="https://www.contena.cn">Hello World</a></h1><p>Some text</p>',
    );

    const renderedLink = canvas.getByRole("link", { name: "Hello World" });
    expect(renderedLink).toHaveAttribute("href", "https://www.contena.cn");
...
export const RemoveLink: MtTextEditorStory = defineStory({
  ...
  args: {
    modelValue: '<h1><a href="https://www.contena.cn">Hello World</a></h1><p>Some text</p>',
  },
  play: async ({ canvasElement, args }) => {
    const canvas = within(canvasElement);
 ...
 * - https://github.com/contena/contena/discussions/11570
 * - https://github.com/contena/contena/issues/11216
 *
