...
`subscribe()` listens for dataset changes and calls your callback every time Contena publishes a new value for the given dataset id.
...
| `callback` | true     | A callback function which will be called every time the Contena Administration publishes the dataset |
