...
export const TestInputValue: MtTextFieldStory = {
  ...
  play: async ({ canvasElement, args }) => {
    ...
    expect((canvas.getByRole("textbox") as HTMLInputElement).value).toBe("Contena");

    expect(args.change).toHaveBeenCalledWith("Contena");
  },
};
...
export const VisualTestCharacterCountExceeding: MtTextFieldStory = {
  ...
  play: async ({ canvasElement, args }) => {
    ...
    await userEvent.type(
      ...
      "Contena is a trendsetting ecommerce platform to power your online business.",
    );
    await userEvent.click(canvas.getByText("hidden"));
