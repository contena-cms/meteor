...
Each Contena app must be hosted on its own domain.
...
If your Contena instance runs inside Docker, use `host.docker.internal:PORT` instead of `localhost:PORT` for the `registrationUrl` and webhook URLs so Contena can reach your app server from inside the container. The `base-app-url` does **not** need this — it is loaded by the browser, not by the Contena server.
:::
...
Optional, but highly recommended for app development, is the [App Server SDK](https://github.com/contena/app-sdk-js): a library written in TypeScript that provides a server setup to simplify the registration handshake, signature verification, webhook handling, and communication with the Contena API.

The App Server SDK supports Node.js, Deno, Bun, and Cloudflare Workers.

Contena also provides SDKs for other languages. See the [full list of official App SDKs](https://developer.contena.cn/docs/guides/plugins/apps/app-sdks/).

To scaffold a new app using the App Server SDK:

```bash
# scaffold the example app
npx tiged contena/app-sdk-js/examples/node-hono demo-app
cd demo-app
...
Visit [the App Server SDK guide](https://developer.contena.cn/docs/guides/plugins/apps/app-sdks/javascript/01-getting_started.html) for detailed instructions.
...
Using the [App Server SDK](https://github.com/contena/app-sdk-js) is recommended, as it handles the registration handshake, signature verification, webhook handling, and request validation. It is also possible to implement the registration and request validation manually for advanced setups.
...
Create the folder `demo-app/meteor-app` with an `index.html` and a `src/main.js`. This is the frontend source that Vite will serve inside the Contena Administration.
...
After the registration handshake is working, add the `<base-app-url>` field inside the `<admin>` section of the [manifest file](https://developer.contena.cn/docs/guides/plugins/apps/app-base-guide#manifest-file). It must point to the path where Vite serves the frontend, so in this example that is `/admin/`.

As required by Contena's app system, the `<setup>` section must already contain the `registrationUrl` and `secret`.

:::tip Docker reminder
Remember: the `registrationUrl` must be reachable by the Contena server. If Contena runs in Docker, use `host.docker.internal` for that URL. The `base-app-url` is loaded by the browser, so `localhost` works fine there.
:::

Create the file at `/custom/apps/MyExampleApp/manifest.xml` in your Contena installation. Example for a local dev setup (app server running on port 3000):

```xml
<?xml version="1.0" encoding="UTF-8"?>
<manifest xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:noNamespaceSchemaLocation="https://raw.githubusercontent.com/contena/platform/trunk/src/Core/Framework/App/Manifest/Schema/manifest-1.0.xsd">
    <meta>
    ...
    <setup>
        <!-- Use host.docker.internal if Contena runs in Docker -->
...
Log in to the Contena Administration. The notification should appear in the top-right corner on any page (the notification is not bound to a specific module — it appears on every page load).
