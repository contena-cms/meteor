...
Staging: https://app.ctpurchaseinterface.staging-services.contena.cn

Production: https://app.ctpurchaseinterface.services.contena.cn
...
- **`options`** (`object`): Configuration options for the payment iframe
  ...
  - **`swVersion`** (`string`): The Contena version
...
- **Success**: An object with at least:
  ...
  - **`aud`**, **`exp`**, **`iat`**, **`iss`**, **`nbf`**, **`swemp`**: Standard JWT and Contena fields
