 ...
 */
import type Repository from "@contena/meteor-admin-sdk/es/data/repository";
import Criteria from "@contena/meteor-admin-sdk/es/data/Criteria";
import { useRepository } from "@contena/meteor-admin-sdk/es/data/composables/useRepository";
import { getRepository } from "@contena/meteor-admin-sdk/es/data/composables/getRepository";
import MtDataTable, {
  type ColumnChanges,
...
const handleItemDelete = async ({ id }: { id: string }) => {
  ...
  if (isInIframe && !props.forceRealModal) {
    const { ui } = await import("@contena/meteor-admin-sdk");
