...
Positions define where UI components can be injected into the Contena Administration.
...
```js
ct.ui.tabs('ct-product-detail').addTabItem({ ... })
```

In this example, `ct-product-detail` is the `positionId` that identifies the tab bar in the product detail page.
