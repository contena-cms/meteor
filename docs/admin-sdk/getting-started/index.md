...
This guide helps you set up the Meteor Admin SDK and start building extensions for the Contena Administration.
...
Contena supports two extension types: **apps** and **plugins**. Both can use the Meteor Admin SDK.

### Apps

Apps run on an external server and communicate with Contena through a defined API. They are the recommended approach because:

- They work with **Contena Cloud** and self-hosted instances, and they are the only extension type available for **Contena SaaS**
- The frontend and backend are fully decoupled from the Contena codebase

Set up an app: [App Installation Flow](./installation-apps.md)

### Plugins

Plugins run directly inside the Contena instance. They have full access to the Contena PHP codebase but are limited to **self-hosted** environments.
