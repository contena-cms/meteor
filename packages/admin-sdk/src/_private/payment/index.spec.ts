    ...
    describe('iframe creation', () => {
      ...
      it('should create an iframe with correct attributes and URL when no valid license', async () => {
        ...
        expect(result.iframeEl.src).toBe(
          'https://example.com/payment?service-name=test-app&service-version=1.0.0&shop-url=https://shop.example.com&ct-version=6.5.0&ct-user-language=en-GB&shop-plan=beyond&shop-id=shop-id-123'
        );
      ...
      it('should create an iframe with shop-url and shop-plan from license when license is valid', async () => {
        ...
        expect(result.iframeEl.src).toBe(
          'https://example.com/payment?service-name=test-app&service-version=1.0.0&shop-url=https://license-aud.shop.example.com&ct-version=6.5.0&ct-user-language=en-GB&shop-plan=premium&shop-id=shop-id-123'
        );
      });
    ...
    describe('event handling', () => {
      ...
      it('should handle PAYMENT_CLOSE event and close modal', async () => {
        ...
        expect(mockModalClose).toHaveBeenCalledWith({
          locationId: 'ct-service-payment-modal',
        });
      });
