...
:::caution
The consent API is experimental. Behavior, payloads, and availability may still change in upcoming Contena versions.
:::

> Available since Contena v6.7.10.0

The `ct.consent` API can be used to inspect the current state of a consent and to trigger a consent request flow in the Administration.

`ct.consent.status()` returns a `Promise<Consent>`.
`ct.consent.request()` returns an object with:
- `requestPromise: Promise<Consent>`
- `abort: (reason: unknown) => void`

## `Consent`

`Consent` is the value object returned by the consent API. It represents the current state of a consent in Contena.
...
```ts
const consent = await ct.consent.status({
...
```ts
const { requestPromise } = ct.consent.request({
  consent: 'product_analytics',
  requestMessage: 'Please confirm that we may track Admin usage data.',
  privacyLink: 'https://www.contena.cn/en/privacy/dpa/',
});
...
`requestPromise` resolves when Contena sends back the matching consent response.

#### Abort example

```ts
const { requestPromise, abort } = ct.consent.request({
  consent: 'product_analytics',
});
