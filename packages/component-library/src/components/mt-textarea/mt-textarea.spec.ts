...
describe("mt-textarea", () => {
  ...
  it("emits the changed value when the user moves focus to another element", async () => {
    ...
    // ACT
    await userEvent.type(screen.getByRole("textbox"), "Contena");
    await userEvent.tab();

    // ASSERT
    expect(handler).toHaveBeenNthCalledWith(1, "Contena");
  });
  ...
  it("does not allow typing more characters than the specified max length", async () => {
    ...
    // ASSERT
    expect(screen.getByRole("textbox")).toHaveValue("Cont");
    expect(handler).toHaveBeenNthCalledWith(4, "Cont");
  });
  ...
  it("is possible to edit value of textarea with unlinked inheritance", async () => {
    ...
    // ASSERT
    expect(screen.getByRole("textbox")).toHaveValue("Contena");
