...
describe("mt-url-field", () => {
  ...
  it("updates the domain when the user types", async () => {
    ...
    // ACT
    await userEvent.type(screen.getByRole("textbox"), "www.contena.cn");

    // ASSERT
    expect(screen.getByRole("textbox")).toHaveValue("www.contena.cn");

    expect(handler).toHaveBeenCalledTimes(14);
    expect(handler).toHaveBeenNthCalledWith(1, "https://w");
    expect(handler).toHaveBeenNthCalledWith(14, "https://www.contena.cn");
  });
  ...
  it("updates the domain when the user types and then focuses another element", async () => {
    ...
    expect(handler).toHaveBeenCalledWith("www.contena.cn");
  });
