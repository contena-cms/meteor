...
.admin-shell__main { min-width: 0; margin-left: var(--admin-sidebar-width); transition: margin-left 160ms ease; }
.admin-shell--collapsed .admin-shell__main { margin-left: var(--admin-sidebar-collapsed-width); }
.admin-breadcrumb { height: 40px; display: flex; align-items: center; gap: var(--scale-size-8); padding: 0 var(--scale-size-24); color: var(--color-text-secondary-default); border-bottom: 1px solid var(--color-border-secondary-default); background: var(--color-elevation-surface-default); font-size: var(--font-size-xs); }
.admin-breadcrumb a { color: var(--color-text-primary-default); font-weight: var(--font-weight-semibold); }
.admin-content { min-width: 0; padding: var(--scale-size-20) var(--scale-size-24) var(--scale-size-32); }
